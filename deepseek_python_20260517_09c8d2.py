...
class Database:
    ...
    async def get_partial_matches(self, words: List[str]) -> List[Dict]:
        ...
        if self.fallback:
            ...
            cursor = self.db.novels.find({"words": {"$in": words}}).sort("date", -1).limit(10)
            novels = []
    ...
    async def get_all_normalized_names(self) -> List[Tuple[str, str]]:
        """Return list of (id, normalized_name) for fuzzy matching"""
        if self.fallback:
            conn = sqlite3.connect(self.sqlite_path)
            c = conn.cursor()
...
def fuzzy_match(query: str, candidates: List[Tuple[str, str]], cutoff: float = 0.6) -> List[Tuple[str, str, float]]:
    """Return list of (id, name, similarity) for names above cutoff using difflib."""
    results = []
...
async def send_photo_by_novel(context, chat_id: int, novel: Dict):
    """Send the photo associated with a novel, if any."""
...
# ----------------------------------------------------------------------
# Group automatic detection with fuzzy duplicate check
# ----------------------------------------------------------------------
async def handle_group_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 2. Fuzzy matches (even 1% similarity, we use cutoff 0.3 for very loose)
    fuzzy_similar = await find_similar_novels(novel_name, cutoff=0.3)
    if fuzzy_similar:
        # DM the user with similar novels and ask for confirmation
        try:
            await context.bot.send_message(
                chat_id=sender_id,
                text="🔎 **Potential duplicate found!** I found novels with similar names. Please confirm if any is the same.\n"
                     "If you click *No* for all, the novel will be added automatically.\n"
                     "You can also use /cancel to abort."
            )
            for p in fuzzy_similar:
                # send the stored photo
            ...
            # Also store pending add for this group message
            ...
            await update.message.reply_text("🔍 I found similar novels. I’ve DMed you the details. Please check and confirm.")
        except Exception as e:
        ...
        # Still store the novel temporarily? We'll wait for confirmation.
        # But we need to store the photo in storage anyway for later use.
        storage_chat_id, storage_msg_id = await forward_to_storage(context, chat_id, message_id)
        # Store in context but not in DB until confirmed
    ...
    # 3. No duplicates found – add immediately
    storage_chat_id, storage_msg_id = await forward_to_storage(context, chat_id, message_id)
...
async def button_same_diff(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    is_group_pending = (suffix and suffix.isdigit())  # message_id for group
    ...
    if action == "same":
        ...
        if is_manual:
            ...
            if pending:
                ...
                # Also delete the temporarily stored forward? Not needed.
                # Optionally inform group
                try:
                    await context.bot.send_message(chat_id=int(suffix), text="🚨 Duplicate confirmed by user. This novel was not added.")
                except:
                    pass
            else:
                await query.edit_message_text("❌ Duplicate confirmed.")
        else:
            # old style from group photo initial detection (same as above)
            ...
            if exact and GROUP_CHAT_ID != 0:
                ...
                try:
                    await context.bot.send_message(
                        ...
                        text=f"🚨 *Duplicate confirmed* by @{user_esc}\!\nOriginal: {orig_name_esc} by {sender_esc}",
                        parse_mode=ParseMode.MARKDOWN_V2,
        ...
        if is_manual:
            ...
            if pending:
                ...
                if not pending.get("partial_ids"):
                    ...
                    # Add reaction to original group message
                    try:
...
async def add_platform_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text("📺 Now send the *channel name* \(YouTube\):", parse_mode=ParseMode.MARKDOWN_V2, reply_markup=keyboard)
    return CHANNEL
...
async def back_to_channel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text("📺 Now send the *channel name* \(YouTube\):", parse_mode=ParseMode.MARKDOWN_V2, reply_markup=kb)
    return CHANNEL
...
async def finalize_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if fuzzy_similar:
        # show each with image and confirm
        for p in fuzzy_similar:
...
# ----------------------------------------------------------------------
# Search (admins see channel/story; normal users don't, plus view image button)
# ----------------------------------------------------------------------
...
async def show_search_results(update: Update, context, query: str, offset: int):
    ...
    for i, n in enumerate(novels, start=1):
        ...
        # Add inline button to view image
        text += f"   [🖼 View Image](callback_data:viewimg_{n['id']})\n\n"
    text += "Click 'View Image' to see the photo."

    total_pages = (total + limit - 1) // limit
    current_page = offset // limit
    keyboard = build_pagination_keyboard(current_page, total_pages, "search_page")
    # Add view image buttons separately? We'll use inline keyboard per novel, but that's complex in text. Instead use callback_data in message.
    # Simpler: add a separate button row for each novel? Better to use a separate callback query for each image.
    # We'll handle via callback_data viewimg_<id> and send photo.
    if keyboard:
        await update.message.reply_text(text, parse_mode=ParseMode.MARKDOWN_V2, reply_markup=keyboard, disable_web_page_preview=True)
    else:
...
async def search_page_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    query_esc = escape_mdv2(query_text)
    header_esc = escape_mdv2(f"({offset+1}-{min(offset+limit, total)} of {total}):")
    text = f"🔍 Results for *{query_esc}* {header_esc}\n\n"
...
async def handle_bulk_import_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for line in lines:
        ...
        exact = await db.get_exact_match(norm_name)
        if not exact:
            # Also check fuzzy duplicates? For bulk import we can skip to keep simple
            await db.add_novel(
...
# ----------------------------------------------------------------------
# Indexing (connects standalone text to standalone photo) with duplicate check
# ----------------------------------------------------------------------
...
async def process_indexed_novel(update: Update, context, caption_text: str, photo_msg):
    ...
    if exact:
        ...
        return
    # Fuzzy check
    fuzzy = await find_similar_novels(novel_name, cutoff=0.3)
    if fuzzy:
        await update.message.reply_text(f"⚠️ Similar novels found for '{novel_name}'. Skipping to avoid duplicates. Use manual add if needed.")
        return
...
def main():
    ...
    # Process group photos globally
    app.add_handler(MessageHandler(filters.PHOTO & (filters.ChatType.GROUPS | filters.ChatType.SUPERGROUP), handle_group_photo))
    app.add_handler(MessageHandler((filters.TEXT | filters.PHOTO | filters.FORWARDED) & filters.ChatType.PRIVATE & ~filters.COMMAND, handle_private_message))
