...
class Database:
    ...
    async def add_novel(self, normalized_name, original_name, author, platform, channel, story_name,
        ...
        if self.fallback:
            ...
            res = await self.db.novels.insert_one(doc)
...
def normalize(name: str) -> str:
    ...
    # Robust alphanumeric script matching (including multi-language Unicode / Hindi character properties)
...
async def handle_group_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if exact:
        ...
        date_esc = escape_mdv2(exact['date'][:10])
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📥 Export Database (CSV)", callback_data="admin_export")]
    ])
    if update.callback_query:
...
async def generate_and_send_export(chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    bio.name = f"novels_export_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.csv"
