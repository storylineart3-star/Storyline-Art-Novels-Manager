...
def normalize(name: str) -> str:
    ...
    # Removes punctuation but keeps Hindi/Universal characters and spaces intact
...
# ----------------------------------------------------------------------
# Group automatic detection (Now responds to ANY group context)
# ----------------------------------------------------------------------
async def handle_group_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Process only if there's a photo
    if not update.message or not update.message.photo:
        return
    # If GROUP_CHAT_ID is set, ensure it only processes for that specific chat ID
...
async def show_admin_list(update: Update, context):
    ...
    await update.callback_query.edit_message_text(text, parse_mode=ParseMode.MARKDOWN_V2, reply_markup=InlineKeyboardMarkup(keyboard_arr))
...
async def handle_bulk_import_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for line in lines:
        ...
        # Remove leading numbers, brackets, quotes, trailing checkmarks, and extra spaces
        line = re.sub(r'^(?:\d+[\.\)]\s*)?[\(\"\']+|[\)\"\'\✓\s]+$', '', line).strip()
        if not line: 
            continue

        # Automatically detect splitters like ' = ', ' - ', ' -; ' etc.
        parts = re.split(r'\s*(?:=|[-;]+)\s*', line, maxsplit=1)
        
        novel_name = parts[0].strip()
        story_name = parts[1].strip() if len(parts) > 1 else None

        # Clean trailing artifacts just in case
...
def main():
    ...
    # Process group photos globally to handle any chat/supergroup scenario
