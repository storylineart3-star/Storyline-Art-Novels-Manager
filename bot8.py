...
async def handle_group_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if exact:
        ...
        date_esc = escape_mdv2(exact['date'][:10]) if exact.get('date') else '—'
...
# ----------------------------------------------------------------------
# Manual add conversation (with skip, back, and cancel navigation)
# ----------------------------------------------------------------------
async def add_novel_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query:
        await query.answer()
        await query.edit_message_text("📖 Send me the *novel name*:", parse_mode=ParseMode.MARKDOWN_V2, reply_markup=keyboard)
    else:
        await update.message.reply_text("📖 Send me the *novel name*:", parse_mode=ParseMode.MARKDOWN_V2, reply_markup=keyboard)
    return NAME

async def add_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["novel_name"] = update.message.text.strip()
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("⬅️ Back", callback_data="back_to_name"),
         InlineKeyboardButton("⏭️ Skip", callback_data="skip_author")],
        [InlineKeyboardButton("❌ Cancel", callback_data="cancel_add")]
    ])
    await update.message.reply_text("✍️ Now send the *author name*:", parse_mode=ParseMode.MARKDOWN_V2, reply_markup=keyboard)
    return AUTHOR
...
async def back_to_author(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text("✍️ Now send the *author name*:", parse_mode=ParseMode.MARKDOWN_V2, reply_markup=keyboard)
    return AUTHOR
...
async def add_channel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("⬅️ Back", callback_data="back_to_channel"),
         InlineKeyboardButton("⏭️ Skip", callback_data="skip_story")],
        [InlineKeyboardButton("❌ Cancel", callback_data="cancel_add")]
    ])
    await update.message.reply_text("📝 Now send the *story name on that channel*:", parse_mode=ParseMode.MARKDOWN_V2, reply_markup=keyboard)
    return STORY_NAME
...
async def back_to_story(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text("📝 Now send the *story name on that channel*:", parse_mode=ParseMode.MARKDOWN_V2, reply_markup=keyboard)
    return STORY_NAME

async def go_to_image(reply_func, context):
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("⬅️ Back", callback_data="back_to_story"),
         InlineKeyboardButton("⏭️ Skip", callback_data="skip_image")],
        [InlineKeyboardButton("❌ Cancel", callback_data="cancel_add")]
    ])
    await reply_func("🖼️ Send a *photo*, or use the buttons below:", reply_markup=keyboard, parse_mode=ParseMode.MARKDOWN_V2)
    return IMAGE
