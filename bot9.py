...
async def handle_group_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message or not update.message.photo:
        return
    # Ensure it only fires in the target group if ID is set
...
async def bulk_import_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Send the text list of novels. Format:\n"
        "1. Novel Title = Story Name\n"
        "2. Another Title\n"
        "(The '=' and Story Name are optional)"
    )
...
async def handle_bulk_import_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for line in lines:
        ...
        if match:
            ...
            story_name = match.group(2).strip() if match.group(2) else None
            norm_name = normalize(novel_name)
    ...
    await update.message.reply_text(f"✅ Bulk import complete. {added} unique novels added successfully.")
