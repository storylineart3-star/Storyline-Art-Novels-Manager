        ...
        for p in partials:
            try:
                ...
                if fwd_chat_id and fwd_msg_id:
                    ...
                    id_esc = escape_mdv2(str(p['id']))
                    name_esc = escape_mdv2(p['original_name'])
                    await context.bot.send_message(chat_id=chat_id,
                                                   text=f"Similar novel \\(ID {id_esc}\\): {name_esc} \\(no image\\)",
                                                   parse_mode=ParseMode.MARKDOWN_V2)
            except:
                pass
            keyboard = InlineKeyboardMarkup([[
...
async def show_admin_list(update: Update, context):
    ...
    if not novels:
        ...
        await update.callback_query.edit_message_text("No novels found.", reply_markup=keyboard)
        return
    ...
    keyboard_layout = []
    if buttons:
        keyboard_layout.append(buttons)
    keyboard_layout.append([InlineKeyboardButton("🔧 Set filter", callback_data="admin_filter")])
    keyboard_layout.append([InlineKeyboardButton("🔙 Back to Admin", callback_data="admin")])
    
    await update.callback_query.edit_message_text(text, parse_mode=ParseMode.MARKDOWN_V2, reply_markup=InlineKeyboardMarkup(keyboard_layout))
...
async def admin_filter_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("❌ Clear Filters / Skip", callback_data="admin_clear_filters")],
...
async def admin_clear_filters_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    context.user_data["admin_list_filters"] = {}
    context.user_data["admin_list_offset"] = 0
    context.user_data["awaiting_filter"] = False
    await query.edit_message_text("Filters cleared\\.", parse_mode=ParseMode.MARKDOWN_V2)
    await show_admin_list(update, context)
...
async def admin_delete_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back to Admin", callback_data="admin")]])
    await query.edit_message_text("Send the ID of the novel to delete:", reply_markup=keyboard)
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(admin_clear_filters_callback, pattern="^admin_clear_filters$"))
