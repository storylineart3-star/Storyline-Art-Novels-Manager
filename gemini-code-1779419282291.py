...
async def process_group_submission(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str, photo_data: Optional[dict] = None):
    ...
    # 2. Fuzzy matches (ask user in DM) - UPDATED CUTOFF
    fuzzy_similar = await find_similar_novels(novel_name, cutoff=0.75)
    if fuzzy_similar:
        try:
...
async def universal_group_scanner(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Scans every message in the group, including photos, videos, text, 
    forwards, and documents to trigger your processing logic.
    """
    msg = update.message
    if not msg:
        return

    # Check if it's a reply to the bot's prompt for a title (photo queue logic)
    if msg.reply_to_message and msg.reply_to_message.from_user.id == context.bot.id:
        reply_id = msg.reply_to_message.message_id
        queue = context.chat_data.get("group_photo_queue", {})
        if reply_id in queue:
            photo_data = queue.pop(reply_id)
            await process_group_submission(update, context, msg.text, photo_data)
            return

    # Extract text content from either caption or message text
    text_content = msg.caption or msg.text or ""
    
    # Process only if there is actual text content (we don't process empty photos blindly anymore without asking for a title)
    if text_content:
        await process_group_submission(update, context, text_content)
    elif msg.photo:
        # If it's a photo without a caption, queue it and ask for a title
