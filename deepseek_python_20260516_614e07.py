...
async def forward_to_storage(context: ContextTypes.DEFAULT_TYPE, chat_id: int, message_id: int):
    """Forward a message to the storage group and return (storage_chat_id, storage_message_id) or None."""
...
async def handle_group_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if exact:
        ...
        # Try to forward from storage or original
    ...
    if partials:
        try:
            ...
            for p in partials:
                # Forward original (from storage or original)
        ...
        if is_manual:
            ...
            if pending:
                ...
                if not pending.get("partial_ids"):
                    # All partials resolved, add novel
                    await db.add_novel(
...
async def add_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Also save the message for storage forwarding
...
async def finalize_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Forward to storage if image present
...
# ----------------------------------------------------------------------
# Indexing old posts (supports pairing text + photo)
# ----------------------------------------------------------------------
async def index_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data["pending_index"] = None  # will store either 'text' or 'photo'
    await update.message.reply_text(
        "📥 Index mode ON. Forward old group posts to me.\n"
        "You can forward a single photo with caption, or a text (novel name) followed by a photo.\n"
...
async def handle_forward(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.user_data.get("index_mode", False):
        return
    msg = update.message
    # Only accept forwarded messages
    if not msg.forward_origin:
        await msg.reply_text("Please forward messages from the group.")
        return
    # Check origin group
    if hasattr(msg.forward_origin, "chat"):
        ...
        await msg.reply_text("Cannot determine original chat. Ignoring.")
        return
    ...
    if msg.photo:
        ...
        caption = msg.caption
        if caption:
            # Photo with caption – process directly
            await process_indexed_novel(update, context, caption, msg)
        else:
            # Photo without caption – try to pair with pending text
            if pending and pending["type"] == "text":
                novel_name = pending["caption"]
                await process_indexed_novel(update, context, novel_name, msg)
                context.user_data["pending_index"] = None
            else:
                # Store this photo as pending
                context.user_data["pending_index"] = {
                    "type": "photo",
                    "message": msg
                }
                await msg.reply_text("Photo received. Waiting for the novel name (text).")
    elif msg.text:
        # Text message (novel name)
        if pending and pending["type"] == "photo":
            # Pair with pending photo
...
async def process_indexed_novel(update: Update, context, caption_text: str, photo_msg):
    """Save a novel from a forwarded photo message (or photo + separate text)."""
