...
# ----------------------------------------------------------------------
# Group automatic detection – FIXED VERSION
# ----------------------------------------------------------------------
async def handle_group_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if GROUP_CHAT_ID != 0 and msg.chat_id != GROUP_CHAT_ID:
        return
    ...
    partials = await db.get_partial_matches(norm_name)

    if partials:
        text = "⚠️ Similar novels found:\n\n"

        for p in partials[:10]:
            text += (
                f"📖 {p['original_name']}\n"
                f"🔍 Match: {round(p.get('match_score', 0), 2)}%\n\n"
            )

        text += "Reply YES to force add or NO to cancel."

        await msg.reply_text(text)

        context.user_data[f"group_pending_{message_id}"] = {
            "norm_name": norm_name,
...
# ----------------------------------------------------------------------
# Indexing (connects standalone text to standalone photo) – FIXED VERSION
# ----------------------------------------------------------------------
...
async def process_indexed_novel(update: Update, context, caption_text: str, photo_msg):
    ...
    if partials:
        text = "⚠️ Similar novels found during indexing:\n\n"
        ...
        context.user_data["pending_force_index"] = {
            "norm_name": norm_name,
            "novel_name": novel_name,
            "author": author,
            "file_id": file_id,
    ...
    await update.message.reply_text(
        f"✅ Indexed: {escape_mdv2(novel_name)}",
        parse_mode=ParseMode.MARKDOWN_V2
    )

# ----------------------------------------------------------------------
# Primary Message Router (with pending_force_index handling)
# ----------------------------------------------------------------------
async def handle_private_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ud = context.user_data
    msg = update.message

    # NEW: handle forced indexing confirmation
    if ud.get("pending_force_index"):
        text = msg.text.strip().lower()
        pending = ud["pending_force_index"]

        if text == "yes":
            await db.add_novel(
                normalized_name=pending["norm_name"],
                original_name=pending["novel_name"],
                ...
                chat_id=pending["chat_id"],
                message_id=pending["message_id"],
                storage_chat_id=pending["storage_chat_id"],
                storage_message_id=pending["storage_message_id"],
                sender_id=update.effective_user.id,
                sender_name="indexed",
                added_by_index=1
            )
            await msg.reply_text("✅ Force added successfully.")
        else:
            await msg.reply_text("❌ Add cancelled.")

        ud.pop("pending_force_index", None)
        return
...
def main():
    ...
    # FIXED GROUP LISTENER: handles photos, text, captions, forwards, media groups
    app.add_handler(
        MessageHandler(
            (
                filters.PHOTO |
    ...
    # IMPORTANT REMINDER: Bot must have privacy mode DISABLED via @BotFather (/privacy -> Disable)
    logger.info("Bot started. Make sure privacy mode is disabled in BotFather to receive all group messages.")
